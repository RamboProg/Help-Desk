    ...
    <div className='flex min-h-screen'>
            ...
            {tickets.length > 0 ? (
              tickets.map((ticket, index) => (
                <div key={index} className='mb-2 bg-white p-4 rounded-md shadow-md'>
                  ...
                  <p className={`text-${theme.colors.text} mb-4`}>{ticket.Description}</p>
                  {canStartChat(ticket) && (
                    <button onClick={() => handleStartChat(ticket)} className={`bg-${theme.colors.primary} py-2 px-4 rounded-full`}>
                      Start Chat
                    </button>
                  )}
                </div>
              ...
              <p className={`text-${theme.colors.text}`}>No tickets found.</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className='flex-1 p-8 mt-8'>
        <div className='max-w-[1640px] mx-auto bg-gray-200 bg-opacity-50 rounded-lg p-8 flex flex-col'>
          <h2 className={`text-${theme.colors.primary} font-extrabold text-4xl mb-8 border-b-4 border-${theme.colors.primary} pb-4`}>Rate Agents</h2>
          {closedTickets.length > 0 ? (
                ...
                !isTicketRated(ticket._id) && (
                  <div key={index} className='mb-2 bg-gray p-4 rounded-md shadow-md flex items-center justify-between'>
                    ...
                    <div className='flex items-center'>
                      <label htmlFor={`rating-${ticket._id}`} className={`text-${theme.colors.text} mr-2`}>
                        Rate:
                      </label>
            ...
            <p className={`text-${theme.colors.text}`}>No closed tickets found to rate.</p>
          )}
        </div>
      </div>
    </div>
